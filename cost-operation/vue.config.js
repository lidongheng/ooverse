...
module.exports = defineConfig({
  ...
  // 本地开发使用根路径，生产环境部署到统一镜像中的 cost-operation 子目录。
  publicPath: process.env.NODE_ENV === 'production' ? '/ooverse/cost-operation/' : '/',
  outputDir: 'dist/cost-operation',
  assetsDir: 'static',
  lintOnSave: false,
    ...
    if (process.env.NODE_ENV === 'production') {
      config.plugins.push(
        new FileManagerPlugin({
          onStart: {
            delete: ['./dist/cost-operation.zip']
          },
          onEnd: {
            archive: [
              {
                source: './dist/cost-operation',
                destination: './dist/cost-operation.zip'
              }
            ]
          }
        })
